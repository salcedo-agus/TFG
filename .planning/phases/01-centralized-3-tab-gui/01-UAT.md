...
### 2. Mission-inputs walkthrough (re-test after G-01-2 fix)
expected: Run with a valid combo works; results show real mass values (not null); Delta-V line equals the label; Save filename follows `staging_{n}stage_dv{dv:.1f}_pl{pl}kg.txt`.
result: pass
verified: 2026-08-15 — masses render real/finite after Run; no null/NaN
...
### 4. Phase gate `python SRC/test_call.py` (re-test after G-01-4 fix)
expected: Bridge smoke runs from the repo root (DLL built); prints finite real masses; behavior unchanged by construction (zero diff in gui.py:22-110).
result: pass
verified: 2026-08-15 — ran from repo root; all 3 stages finite (m0=207902.0 / 27759.5 / 8122.1 kg, mp/ms real); exit 0
